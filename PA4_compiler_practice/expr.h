...
class Expr { // abstract base class for expressions
	protected:
		...
		int stacksize;
	public:
		Expr(Expr_Type type_) : type(type_), stacksize(0){}
		Expr(Expr_Type type_, int stacksize_) : type(type_), stacksize(stacksize_) {}
		Expr_Type getType() const;
		void setType(Expr_Type type_);
		virtual void gencode(FILE *f) const = 0; // pure virtual method
		int getStacksize(); 
		void setStacksize(int size_);
};
...
class IntConstExpr : public Expr {
	protected:
		int value;
	public:
		IntConstExpr(int value_) : Expr(INT_TYPE, 1), value(value_){}
...
class BoolConstExpr : public Expr {
	protected:
		bool value;
	public:
		BoolConstExpr(bool value_) : Expr(BOOL_TYPE, 1), value(value_){}
		void gencode(FILE* f) const;
};
...
class FloatConstExpr : public Expr {
	protected:
		float value;
	public:
		FloatConstExpr(float value_) : Expr(FLOAT_TYPE, 1), value(value_){}
		void gencode(FILE* f) const;
};
