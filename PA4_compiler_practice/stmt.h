...
class Stmt { // abstract base class for statements
	...
	public:
		Stmt() : stacksize(0) {}
		Stmt(int stacksize_) : stacksize(stacksize_) {}
...
class ExprStmt : public Stmt {
	protected:
		Expr* expr;
	public:
		ExprStmt(Expr* expr_) : expr(expr_) {setStacksize( expr->getStacksize() );}
...
class CompoundStmt : public Stmt {
	...
	public:
		CompoundStmt(vector<Expr*>* localDecls_, vector<Stmt*>* stmts_);
		virtual void gencode(FILE *f) const;
};
